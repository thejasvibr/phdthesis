# ```itsfm```, an open-source package to reliably segment and measure sounds by frequency modulation {#itsfmchapter}
...
*Beleyur, T. (2020). ```itsfm```, an open-source package to reliably segment and measure sounds by frequency modulation bioRxiv 2021.01.09.426033; doi: https://doi.org/10.1101/2021.01.09.426033 *
...
```{r cffmseg, echo=FALSE, out.width="100%",fig.cap="Diagnostic plot showing the CF/FM segmentation output of a \\textit{Rhinolophus euryale/mehelyi} call"}
include_graphics('original_papers/itsfm-paper/figures/pwvd_cffm_segmentation.png')
```
...
```{r pkpctgdiags, echo=FALSE,out.width="100%",fig.cap="Diagnostic output showing the underlying basis of the peak-percentage CF-FM segmentation method. Top: A spectrogram representation of the call shown in Figure 1, Below: the high/low-passed dB rms profiles of the call. The peak frequency of the entire call is taken and a high and low-passed version of the sound if created at 99$\\%$ of the call peak frequency. The dB rms profile differences of the high and low-passed sounds are calculated and subtracted from each other. The region where the low-passed dB rms profile is higher are labelled FM and vice-versa as CF. "}
include_graphics('original_papers/itsfm-paper/figures/pkpctage_profiles.png')
```
...
Table: (\#tab:synthtable) Parameter values used to generate synthetic CF-FM calls. The parameters broadly reflect the call shape of a rhinolophid/hipposiderid CF-FM bat calls. iFM and tFM regions were generated from the same FM parameter set. 9 CF x 6 iFM  x 6 tFM combinations = 324 calls

|Parameter name         | Values   |
|-----------------------|----------|
|CF duration (ms)       |5, 10, 15 |
|CF peak frequency (kHz)|40, 60, 90|
|i/tFM duration (ms)    |1,2       |
|i/tFM bandwidth (kHz)  | 5,10,20  |
...
Table: (\#tab:segparams) Segmentation method specific parameters used to analyse the synthetic data.

+-------------------------+---------------------+--------------------------+-------------------------------------+-----------------------+
|Method                   |Parameters           |                          |                                     |                       |
+=========================+=====================+==========================+=====================================+=======================+
|   pwvd                  |                     |                          |                                     |                       |
|                         |Window size (samples)|FM rate threshold (kHz/ms)|Accelaration threshold (kHz/ms$^{2}$)|Extrapolation window(s)|
|                         |                     |                          |                                     |                       |
|                         |     125             |       2                  |   10                                |  75x $10^{-6}$        |
+-------------------------+---------------------+--------------------------+-------------------------------------+-----------------------+
|   peak percentage       |                     |                          |                                     |                       |
|                         |Window size (samples)|Peak percentage           |Double pass                          |                       |
|                         |                     |                          |                                     |                       |
|                         |     125             |          0.99            |       True                          |                       |
+-------------------------+---------------------+--------------------------+-------------------------------------+-----------------------+
...
include_graphics('original_papers/itsfm-paper/figures/pwvd-pkpct-comparison.png')
```

```{r accuracypctiles, echo=FALSE}

d <- read.csv("original_papers/itsfm-paper/data/accuracy_pctiles.csv")
d$region.type <- c("CF","CF ","tFM","tFM ","iFM","iFM ")
d$method <- c("peak percentage", "pwvd","peak percentage ", "pwvd","peak percentage", "pwvd")
d$X0 <- as.character(d$X0)

remove.brackets <- function(in.str){
  round1<- gsub('[',"",in.str,fixed=TRUE)
  round2<- gsub(']',"",round1,fixed=TRUE)
  round2
  }
remove.brackets.and.split.by.space <- function(input.string){
                      split.words <- strsplit(input.string, " +")
                      first  <- remove.brackets(split.words[[1]][1])
                      second <- remove.brackets(split.words[[1]][2])
                      c(first, second)
                                     }

d$lower <- NA 
for (each in seq(length(d$X0))){ d$lower[each]<-remove.brackets.and.split.by.space(d$X0[each])[1]}
d$lower <- round(as.numeric(d$lower),2)
d$higher <- NA 
for (each in seq(length(d$X0))){ d$higher[each]<-remove.brackets.and.split.by.space(d$X0[each])[2]}
d$higher <- round(as.numeric(d$higher),2)

```


|  Call component | Segmentation method   | Segmentation accuracy (95$\%$ile range)  |
|-----------------|-----------------------|------------------------------------------|
|CF               | peak percentage       |  `r d$lower[1]`-`r d$higher[1]`          |
|CF               |    pwvd               |  `r d$lower[2]`-`r d$higher[2]`          |
|tFM              |peak percentage        |  `r d$lower[3]`-`r d$higher[3]`          |
|tFM              |    pwvd               |  `r d$lower[4]`-`r d$higher[4]`          |
|iFM              |peak percentage        |  `r d$lower[5]`-`r d$higher[5]`          |
|iFM              |    pwvd               |  `r d$lower[6]`-`r d$higher[6]`          |

Table: (\#tab:accuracypctiles) Summary statistics describing the performance of the two segmentation methods on the synthetised test data set.  The pwvd method performs better than the peak-percentage method over the tested parameter space and for all call components (iFM,tFM and CF). The relative segmentation accuracy is defined as $\frac{measured \:duration}{original \:duration}$
...
The use of ```itsfm``` in the analysis of other types of vocalisations still needs further exploration. For instance, bird calls have been analysed (See [online user-guide](https://itsfm.readthedocs.io/en/latest/gallery_dir/z_bird_eg.html#sphx-glr-gallery-dir-z-bird-eg-py)). The current 'pwvd' frequency tracking implementation only tracks a single frequency per point of time, and thus is not able to handle multi-harmonic sounds with equal harmonic emphasis very well. Future implementations of frequency tracking need to apply more sophisticated problem-region detection and also frequency tracking (eg. Viterbi path).

## Open-source software and packages used
```itsfm``` is written in the Python language [@van1995python], and relies on the numpy, scipy, pandas, matplotlib and tftb [@numpy;@2020SciPy;@hunter2007a;@pandas;@tftb]. The Jupyter Notebook and Rmarkdown projects [@jupyter;@rmarkdown] were used in the analysis of data and writing of this paper. 
